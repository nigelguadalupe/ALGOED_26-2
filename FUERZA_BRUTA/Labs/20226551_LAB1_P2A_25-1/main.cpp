...
void imprimirCromosoma(int*cromosoma,int cantRecursos,int costosTotales) {
    ...
    cout <<"| Costo: "<< costosTotales<<" miles"<<endl;
}

void llenarTotales(int*cromosoma,int cantRecursos,Recurso*recursosSeguridad,int&costosTotales) {
...
void explorarCombinaciones(Recurso*recursosSeguridad,int numCombinaciones,int cantRecursos,int presupuesto) {

    cout << "Combinaciones validas (Costo entre "<<presupuesto*0.80<<" y "<<presupuesto<<"):"<<endl;
    for (int valor = 0;valor<=numCombinaciones;valor++) {
        ...
        if (combinacionValida(cromosoma, cantRecursos, recursosSeguridad)) {
            llenarTotales(cromosoma,cantRecursos,recursosSeguridad,costosTotales);
            if (costosTotales <= presupuesto and costosTotales >= presupuesto*0.8) {
                cout << "Combinacion "<<valor<<":";
                imprimirCromosoma(cromosoma,cantRecursos,costosTotales);
            }
        }
...
int main() {
    ...
    explorarCombinaciones(recursosSeguridad,numCombinaciones,CANT_MAX,presupuesto);
